...
class WaveletTransformer(Model):
    ...
    def call(self, x):
        ...
        x = tf.reshape(x, [-1, 500, 40])
        x, cD = tf.numpy_function(lambda x: pywt.dwt(x, 'haar', mode='symmetric', axis=-1), [x], [tf.float32, tf.float32]) # use lambda function to call pywt.dwt with specified arguments
